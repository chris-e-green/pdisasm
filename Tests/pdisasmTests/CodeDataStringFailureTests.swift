...
final class CodeDataStringFailureTests: XCTestCase {
    ...
    func testReadStringDecodingFailed() {
        ...
        var cd = CodeData(data: Data([0x02, 0x80, 0x81]), instructionPointer: 0, header: 0)
    ...
    func testReadStringSucceedsWithValidASCII() throws {
        var cd = CodeData(data: Data([0x03, 0x41, 0x42, 0x43]), instructionPointer: 0, header: 0)
        let result = try cd.readString()
        XCTAssertEqual(result, "ABC")
    }

    func testReadStringEmptyString() throws {
        var cd = CodeData(data: Data([0x00]), instructionPointer: 0, header: 0)
        let result = try cd.readString()
    ...
    func testReadStringTruncated() {
        ...
        var cd = CodeData(data: Data([0x05, 0x41, 0x42]), instructionPointer: 0, header: 0)
        XCTAssertThrowsError(try cd.readString()) { error in
