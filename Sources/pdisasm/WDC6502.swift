...
let wdc6502: [UInt8: Wdc6502OpInfo] = [
    ...
    0xc4:Wdc6502OpInfo(mnemonic:"CPY ($%02x),Y",paramLength: 1),
    ...
    0xd1:Wdc6502OpInfo(mnemonic:"CMP $%02x",paramLength: 1),
    ...
    0xec:Wdc6502OpInfo(mnemonic:"CPX $%02x",paramLength: 1),
...
func decodeAssemblerProcedure(procedureNumber:Int, proc: inout Procedure, code: Data, addr: Int) {
                    proc.header = "ASMPROC\(procedureNumber)"
...
// TODO: adjust relative locations in the code so that they make sense to reader
                    ...
                    repeat {
                        if let opcode = wdc6502[op] {
                            ...
                            proc.instructions[ipc] = machCodeStr + String(format: opcode.mnemonic, param)
                            ipc += 1
                            if procRelocs.contains(ipc) && opcode.paramLength > 0 {
                                proc.instructions[ipc - 1]? += " <- proc relocated"
                            }
                    ...
                    while i < (addr - pos) {
                        if i.isMultiple(of: 16) && !s.isEmpty {
                            proc.instructions[((i - 1) / 16) * 16] = s + " | " + sh
                            s = ""
                            sh = ""
                        }
                        if procRelocs.contains(i) {
                            s += String(format: " *%04x", code.readWord(at: i))
                            sh += "  "
                            i += 2
                        } else {
                    ...
                    if !s.isEmpty {
                        proc.instructions[((addr - pos - 1) / 16) * 16] = s + String(repeating: " ", count: (48 - s.count)) + " | " + sh
                    }
}