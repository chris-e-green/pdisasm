public class SegDictionary: CustomStringConvertible {
    ...
    public var description: String {
        ...
        let tableRows = sortedSegments.map { (slot, segment) in
            return
                "| \(slot) | \(segment.segNum) | \(segment.name) | \(String(format:"%04X", segment.codeaddr)) | \(segment.codeleng) | \(segment.segkind) | \(String(format:"%04X", segment.textaddr)) | \(segment.mType) | \(segment.version) |"
        }
