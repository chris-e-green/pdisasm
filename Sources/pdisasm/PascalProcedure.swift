        ...
        let (address, inst) = sortedInstructions[idx]
            ...
            case fjp:
                ...
                if dest > address {  // jumping forward so an IF
                    if let targetIdx = sortedInstructions.firstIndex(where: {$0.key == dest}) {
                        ...
                        if prevIdx >= sortedInstructions.startIndex {
                            ...
                            if prev.value.opcode == ujp {
                                ...
                                flagForLabel.append((elseDest, indentLevel))
                                if let elseEndIdx = sortedInstructions.firstIndex(where: {$0.key == elseDest}) {
                                    sortedInstructions[elseEndIdx].value.prePseudoCode.append("END")
                                }
                                pseudoCode = "IF \(cond) THEN BEGIN"
                                sortedInstructions[targetIdx].value.prePseudoCode.append("END ELSE BEGIN")
                                ujpToSkipSet.insert(prevIdx)
                            } else {
                                // likely an IF without ELSE
                                sortedInstructions[targetIdx].value.prePseudoCode.append("END")
                                flagForEnd.append((dest, indentLevel))
                                pseudoCode = "IF \(cond) THEN BEGIN"
                            }
                        }
                    ...
                    proc.instructions[dest]?.prePseudoCode.append("REPEAT")
                    pseudoCode = "UNTIL \(cond)"
                }
                proc.entryPoints.insert(dest)
            case inc:
